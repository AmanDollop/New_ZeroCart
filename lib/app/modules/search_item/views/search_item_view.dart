...
class SearchItemView extends GetView<SearchItemController> {
  ...
  Widget categoriesDropdown() {
    return Padding(
      ...
      child: DropdownZeroCart<Categories>(
        wantDivider: false,
        hint: Text(
          "Category",
          style: Theme.of(Get.context!).textTheme.caption?.copyWith(fontSize: 10.px),
        ),
        icon: Icon(Icons.keyboard_arrow_down_rounded,color: MyColorsLight().textGrayColor, size: 18.px),
        selected: controller.categoryObject,
        items: controller.listOfCategories.reversed.map(
  ...
  Widget customizeYourWardrobeButton() {
    return CommonWidgets.myOutlinedButton(
      ...
      onPressed: () => controller.clickOnWardrobe(),
      //width: 65.w,
      height: 42.px,
    );
  }
