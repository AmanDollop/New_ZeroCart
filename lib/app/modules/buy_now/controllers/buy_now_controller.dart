...
class BuyNowController extends CommonMethods {
    ...
    if (inventoryId.isNotEmpty) {
      ...
      if (response != null) {
        if (await CommonMethods.checkResponse(response: response)) {
          getProductByInventoryApiModal = GetProductByInventoryApiModal.fromJson(jsonDecode(response.body));
          if (getProductByInventoryApiModal != null) {
            ...
            if (getProductByInventoryApiModal?.productDetail != null) {
              ...
              if (productDetail?.isOffer != null && productDetail?.isOffer == "1") {
                if (productDetail?.deliveryCharge != null && productDetail!.deliveryCharge!.isNotEmpty && productDetail?.deliveryCharge != "0.0") {
                  deliveryPrice.value = double.parse(double.parse(productDetail!.deliveryCharge!).toStringAsFixed(2));
                }
                if (productDetail?.sellPrice != null) {
                  sellPrice.value = double.parse(productDetail!.sellPrice.toString());
                  sellPriceCopy = sellPrice.value;
                }
                if (productDetail?.sellPrice != null && productDetail?.offerPrice != null) {
                  discountPrice.value = (double.parse(productDetail!.sellPrice.toString()) - double.parse(productDetail!.offerPrice.toString()));
                  discountPriceCopy = discountPrice.value;
                  totalPrice.value = deliveryPrice.value + double.parse(productDetail!.offerPrice.toString());
  ...
  void clickOnIncreaseQuantityButton() {
    MyCommonMethods.unFocsKeyBoard();
    inAsyncCall.value = CommonMethods.changeTheAbsorbingValueTrue();
    if (productDetail?.availability != null && productDetail!.availability!.isNotEmpty) {
