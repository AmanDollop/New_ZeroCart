...
class FilterView extends GetView<FilterController> {
          ...
          body: Obx(
            () {
              ...
              if (CommonMethods.isConnect.value) {
                if (controller.getFilterModal != null && controller.responseCode == 200) {
                  ...
                  () => Row(
                    children: [
                      ...
                      AnimatedContainer(
                        ...
                        height: (controller.initialIndex.value == index) ? 28.px : 0,
                        width: (controller.initialIndex.value == index) ? 3.px : 0,
                          ...
                          children: [
                            Padding(
                              ...
                              child: filtersTextButtonView(
                                text: controller.filterList[index].filterCatName.toString(),
                                onPressed: () => controller.clickOnParticularFilterButton(index: index),
                              ),
                            ),
                          ],
                        ),
                ...
                children: [
                  Obx(
                    () {
                      controller.count.value;
                      if (CommonMethods.isConnect.value) {
                        if (controller.getFilterList != null && controller.responseCode == 200) {
      ...
      return Column(
        children: [
                  ...
                  children: [
                    controller.filterDetailList[index].filterValue != null
                        ? Container(
                            height: 30.px,
                            decoration: BoxDecoration(
                              gradient: controller.filterDetailList[index].filterValue ==
                                      controller.filterData[controller.filterList[controller.initialIndex.value].filterId.toString()]
                                  ? CommonWidgets.commonLinearGradientView()
                                  : null,
                              color: controller.filterDetailList[index].filterValue ==
                                      controller.filterData[controller.filterList[controller.initialIndex.value].filterId.toString()]
                                  ? null
                            ...
                            child: ElevatedButton(
                              onPressed: () => controller.clickOnFilterList(index: index),
                              style: ElevatedButton.styleFrom(padding: EdgeInsets.zero),
                              child: Text(
                                controller.filterDetailList[index].filterValue ?? "",
          ...
          if (controller.filterData.isNotEmpty)
            applyButtonView(
                text: "Apply Filter",
                onPressed: () => controller.clickOnApplyFilterButton(context: context))
        ],
      );
    });
  }

  Widget starIcon() => Image.asset("assets/star.png");

  Widget filtersTextButtonView({required String text, required VoidCallback onPressed}) {
    return SizedBox(
      child: TextButton(
        onPressed: onPressed,
  ...
  Widget applyButtonView({required String text, required VoidCallback onPressed}) {
    return SizedBox(
      height: 40.px,
      width: 40.w,
