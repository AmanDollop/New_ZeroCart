...
class VerificationController extends GetxController {
  ...
  Future<void> afterMatchOtpWorking({required http.Response response}) async {
    if (id == "1") {
      ...
      if (userData.value != null) {
        MyCommonMethods.setString(key: ApiKeyConstant.token, value: userData.value!.customer!.token!);
      ...
      await MyCommonMethods.setString(key: "measurement",value: '');
      selectedIndex.value = 0;
      bottomNavigationController.index=0;
      Get.deleteAll();
  ...
  Future<void> sendOtpApiCalling({required String type}) async {
    ...
    http.Response? response = await CommonApis.sendOtpApi(bodyParams: bodyParams);
    if (response != null) {
      apiResponseMap = jsonDecode(response.body);
      // MyCommonMethods.showSnackBar(message: apiResponseMap[ApiKeyConstant.otp].toString(), context: Get.context!);
      // textEditingController.text = apiResponseMap[ApiKeyConstant.otp].toString();
    }
  }
