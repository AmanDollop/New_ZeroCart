...
import '../../../apis/api_modals/get_product_based_on_history_api_model.dart';
...
class HomeController extends CommonMethods {
  ...
  GetProductBasedOnHistoryApiModel? getProductBasedOnHistoryApiModel;
  List<ProductDetails> getProductBasedOnHistoryApiList = [];

  GetProductBasedOnHistoryApiModel? getProductBasedOnPreferenceApiModel;
  List<ProductDetails> getProductBasedOnPreferenceApiList = [];
  ...
  void setEmpty() {
    ...
    getProductBasedOnHistoryApiModel = null;
    getProductBasedOnHistoryApiModel = null;
    listOfBanner = null;
    getBanner = null;
    listOfBanner?.clear();
    bannerImageList.clear();
    getProductBasedOnPreferenceApiList.clear();
    getProductBasedOnHistoryApiList.clear();
  ...
  Future<void> callingApi() async {
    ...
    await getProductBasedOnHistoryApi();
    await getProductBasedOnPreferenceApi();
  ...
  Future<void> updateFcmIdApiCalling() async {
    ...
    if (fcmId != null && fcmId != "") {
      ...
      http.Response? response = await CommonApis.updateFcmIdApi(bodyParams: bodyParamsForUpdateFcmIdApi);
      responseCode = response?.statusCode ?? 0;
      if (response != null) {}
    }
    increment();
      ...
      if (await CommonMethods.checkResponse(response: response)) {
        ...
        if (topTrendingProducts2Model != null) {
          if (topTrendingProducts2Model?.products != null &&
            ...
            topTrendingProductsList2 = topTrendingProducts2Model?.products ?? [];
          }
        }
      }
    }
  ...
  Future<void> getProductBasedOnPreferenceApi() async {
    ...
    http.Response? response = await MyHttp.getMethod(
        context: Get.context!,
        token: authorization,
        url: ApiConstUri.endPointGetProductBasedOnPreferenceApi);
    responseCode = response?.statusCode ?? 0;
    if (response != null) {
      if (await CommonMethods.checkResponse(response: response)) {
        getProductBasedOnPreferenceApiModel = GetProductBasedOnHistoryApiModel.fromJson(jsonDecode(response.body));
        if (getProductBasedOnPreferenceApiModel != null) {
  ...
  Future<void> getProductBasedOnHistoryApi() async {
    Map<String, String> authorization = {};
    String? token = await MyCommonMethods.getString(key: ApiKeyConstant.token);
    authorization = {"Authorization": token!};
    http.Response? response = await MyHttp.getMethod(
        context: Get.context!,
        token: authorization,
        url: ApiConstUri.endPointGetProductBasedOnHistoryApi);
    responseCode = response?.statusCode ?? 0;
    if (response != null) {
      if (await CommonMethods.checkResponse(response: response)) {
        getProductBasedOnHistoryApiModel =
            GetProductBasedOnHistoryApiModel.fromJson(jsonDecode(response.body));
        if (getProductBasedOnHistoryApiModel != null) {
  ...
  Future<void> getDefaultProductListApi() async {
    ...
    if (response != null) {
      if (await CommonMethods.checkResponse(response: response)) {
        getProductListDefaultForHomeModel = GetProductListForHome.fromJson(jsonDecode(response.body));
        if (getProductListDefaultForHomeModel != null) {
          if (getProductListDefaultForHomeModel?.productList != null && getProductListDefaultForHomeModel!.productList!.isNotEmpty) {
            productListDefault = getProductListDefaultForHomeModel?.productList ?? [];
          }
        }
      }
    }
  ...
  Widget titleTextView() => Text(
        "Permission",
        style: Theme.of(Get.context!).textTheme.subtitle1?.copyWith(fontSize: 20.px),
      );
  ...
  Widget givePermissionTextButtonView() => Text(
        "Give Permission",
        style: Theme.of(Get.context!).textTheme.subtitle2?.copyWith(color: MyColorsLight().success),
      );
  ...
  Future<void> clickOnTailorButton({required BuildContext context}) async {
    await Get.toNamed(Routes.CATEGORIE_PRODUCT, parameters: {
      'categoryName': 'Tailor',
      'categoryId': '0',
      'isChatOption': '',
      'type': 'Tailor',
