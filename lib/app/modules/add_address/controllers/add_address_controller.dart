...
class AddAddressController extends GetxController {
  ...
  Future<void> clickOnSaveAddressButton({required BuildContext context}) async {
    ...
    if (key.currentState!.validate()) {
      try{
        getLatLong = await MyLocation.getLatLongByAddress(address: '${cityController.text.trim()} ${stateController.text.trim()}');
        if (getLatLong != null) {
  ...
  Future<bool> callingAddCustomerAddressApi({required BuildContext context}) async {
   try{
     ...
     http.Response? response = await CommonApis.addCustomerAddressApi(bodyParams: bodyParamsForAddAddressApi);
     if (response != null) {
       return true;
     } else {
       isSaveButtonClicked.value = false;
