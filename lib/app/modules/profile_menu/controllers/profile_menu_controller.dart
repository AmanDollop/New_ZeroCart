...
class ProfileMenuController extends GetxController {
  ...
  Future<void> clickOnArrowIconView({required int index}) async {
    ...
    if (index == 0) {
      ...
      Get.toNamed(Routes.MY_ORDERS,parameters: {'pageName':'ProfileMenu'});
    } else if (index == 3) {
  ...
  Widget cancelTextButtonView() => Text(
        "Cancel",
        style: Theme.of(Get.context!).textTheme.subtitle2?.copyWith(color: MyColorsLight().textGrayColor),
      );

  Widget logOutTextButtonView() => Text(
        "Logout",
        style: Theme.of(Get.context!).textTheme.subtitle2?.copyWith(color: MyColorsLight().error),
      );
