...
class OutfitRoomController extends CommonMethods {
  ...
  Future<void> clickOnSaveButton() async {
    ...
    try{
      ...
      if(upperImagePath.value.isNotEmpty || lowerImagePath.value.isNotEmpty || shoeImagePath.value.isNotEmpty || accessoriesImageList.isNotEmpty){
        addToOutfitsApi();
      }else{
        MyCommonMethods.showSnackBar(message: 'Please select product in outfit.', context: Get.context!);
      }
  ...
  Future<void> moveOutfitToCartApi() async {
    ...
    try{
      ...
      if (response != null && response.statusCode == 200) {
          ...
          bottomNavigationController.index=3;
      }else{
        isAddToCartButtonClicked.value=false;
        MyCommonMethods.showSnackBar(message: 'Please save product in outfit.', context: Get.context!);
      }
    }catch(e){
      isAddToCartButtonClicked.value=false;
      MyCommonMethods.showSnackBar(message: 'Something went wrong!', context: Get.context!);
