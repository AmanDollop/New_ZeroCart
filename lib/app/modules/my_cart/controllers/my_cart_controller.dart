...
class MyCartController extends CommonMethods {
  ...
  Map<String, dynamic> bodyParamsWalletTransactionApi = {};
  ...
  Future<http.Response?> cartItemSelectionApiCalling({String? cartUuid, required String userSelection}) async {
    if (cartUuid != null) {
  ...
  Future<http.Response?> manageCartApiCalling({int? cartQty, CartItemList? cartItem, String? inventoryId}) async {
  ...
  Future<void> clickOnChangeAddressButton({required BuildContext context}) async {
    MyCommonMethods.unFocsKeyBoard();
    inAsyncCall.value = CommonMethods.changeTheAbsorbingValueTrue();
  ...
  Future<void> clickOnFilledCheckedBox({required int index, String? cartUuid}) async {
    MyCommonMethods.unFocsKeyBoard();
    inAsyncCall.value = CommonMethods.changeTheAbsorbingValueTrue();
    ...
    if (response != null) {
      ...
      for (int i = 1; i <= checkedListItemQuantity[index]; i++) {
        if (cartItem?.isOffer != null && cartItem?.isOffer == "1") {
          if (cartItem?.sellPrice != null && cartItem?.offerPrice != null) {
            sellPrice.value = sellPrice.value -
          ...
          if (cartItem!.sellPrice != null && cartItem!.sellPrice!.isNotEmpty) {
            sellPrice.value = sellPrice.value -
                double.parse(double.parse(cartItem!.sellPrice!)
                    .toDouble()
                    .toStringAsFixed(2));
  ...
  Future<void> clickOnReadyToCheckOutItemDownIcon({required int index}) async {
    ...
    if (checkedListItemVariant[index] != null) {
      response = await manageCartApiCalling(cartQty: checkedListItemQuantity[index] - 1, cartItem: cart, inventoryId: checkedListItemVariant[index].inventoryId);
    } else {
      response = await manageCartApiCalling(cartQty: checkedListItemQuantity[index] - 1, cartItem: cart, inventoryId: cart.inventoryId);
    }
    if (response != null) {
      if (cart.isOffer != null && cart.isOffer == "1") {
        if (cart.sellPrice != null && cart.offerPrice != null) {
          sellPrice.value = sellPrice.value - double.parse(double.parse(cart.sellPrice!).toDouble().toStringAsFixed(2));
          discountPrice.value = discountPrice.value - (double.parse(double.parse(cart.sellPrice!).toDouble().toStringAsFixed(2)) - double.parse(double.parse(cart.offerPrice!).toDouble().toStringAsFixed(2)));
        }
      } else {
...
*/
  ...
  Future<void> clickOnReadyToCheckOutItemDeleteIcon({required int itemIndex, String? cartUuid}) async {
    MyCommonMethods.unFocsKeyBoard();
    inAsyncCall.value = CommonMethods.changeTheAbsorbingValueTrue();
    http.Response? response =
  ...
  Future<void> clickOnUnFilledCheckBox({required int index, String? cartUuid}) async {
    MyCommonMethods.unFocsKeyBoard();
    inAsyncCall.value = CommonMethods.changeTheAbsorbingValueTrue();
    http.Response? response = await cartItemSelectionApiCalling(
  ...
  Future<void> clickOnUncheckedItemUpIcon({required int index}) async {
    ...
    http.Response? response;
    if (uncheckedListItemVariant[index] != null) {
      response = await manageCartApiCalling(cartQty: unCheckedListItemQuantity[index] + 1, cartItem: cart, inventoryId: uncheckedListItemVariant[index].inventoryId);
    } else {
      response = await manageCartApiCalling(cartQty: unCheckedListItemQuantity[index] + 1, cartItem: cart, inventoryId: cart.inventoryId);
    }
    if (response != null) {
      unCheckedListItemQuantity[index]++;
    }
  ...
  Future<void> clickOnUncheckedItemDeleteIcon({required int itemIndex, String? cartUuid}) async {
    MyCommonMethods.unFocsKeyBoard();
    inAsyncCall.value = CommonMethods.changeTheAbsorbingValueTrue();
    http.Response? response =
        await removeCartItemApiCalling(cartUuid: cartUuid);
  ...
  Future<void> clickOnProceedToCheckout() async {
    ...
    proceedToPaymentAbsorbing.value = CommonMethods.changeTheAbsorbingValueFalse();
  }
