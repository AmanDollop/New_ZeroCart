...
class CategoryProductController extends CommonMethods {
    ...
    if (await MyCommonMethods.internetConnectionCheckerMethod()) {
      try {
        if (searchPageValue == 'searchPage') {
          ...
          await callCategoryProductApi(type: type,categoryId: categoryId, isChat: isChatOption);
        }
      } catch (e) {
  ...
  Future<void> onLoadMore() async {
    ...
    try {
      if (searchPageValue == 'searchPage') {
        ...
        await callCategoryProductApi(type: type,
  ...
  Future<void> callCategoryProductApi(
    ...
    this.type = type;
    if (((type != null && type.isNotEmpty) || (categoryId != null && categoryId.isNotEmpty)) &&
    ...
    } else if ((type != null && type.isNotEmpty) || (categoryId != null && categoryId.isNotEmpty)) {
      queryParameters = {
        ApiKeyConstant.categoryId: categoryId,
        ApiKeyConstant.type: type,
        ApiKeyConstant.limit: limit.toString(),
  ...
  Future<void> clickOnProduct(
   ...
   await Get.delete<ProductDetail>();
  ...
  Future<void> clickOnFilterButton({
    ...
    if (!isChatOption) {
      ...
      var value = await Get.toNamed(Routes.FILTER,
          arguments: [/*type,*/categoryId ?? "", filterDataMap]);
      if (value != null) {
      ...
      count.value++;
      offset = 0;
      await callCategoryProductApi(type: type, categoryId: categoryId, filterData: filterDataJson);
      inAsyncCall.value = CommonMethods.changeTheAbsorbingValueFalse();
    } else {
      //showAlertDialog();
    }
