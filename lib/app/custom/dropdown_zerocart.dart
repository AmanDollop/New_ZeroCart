...
class DropdownZeroCartButton<T> extends StatefulWidget {
  ...
  @override
  State<DropdownZeroCartButton<T>> createState() =>
      _DropdownZeroCartButtonState<T>();
}
    ...
    return Stack(
      ...
      children: [
        Theme(
          ...
          child: Container(
            ...
            child: DropdownButton(
              ...
              items: widget.items,
              dropdownColor:MyColorsLight().secondary.withOpacity(.8),
              icon: Padding(
                padding: EdgeInsets.only(right: 12.px),
                child: widget.icon,
              ),
        ...
        if (widget.wantDivider)
          Divider(
            thickness: 1,
            color: widget.wantDividerError == true
                ? MyColorsLight().error
